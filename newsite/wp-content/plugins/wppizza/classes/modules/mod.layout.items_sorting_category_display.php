		...
		if(isset($this -> items_keys_categories[$blog_cat_id]) && $this -> items_keys_categories[$blog_cat_id]['category_path'] !=''){
			...
			*/
			if(empty($this->order_category_info[$type][$order_id][$cat_id])){
				...
				*/
				//fix colspan
				$colspan = in_array($type, array('cart', 'gettotals')) ? $colspan : ($colspan-1) ;
				$prepend_category_name['category_' . $cat_id . ''] = '<tr class="'.WPPIZZA_SLUG.'-tr-category"><td class="'.WPPIZZA_SLUG.'-item-category" colspan="'.($colspan).'">'.$this -> items_keys_categories[$blog_cat_id]['category_path'].'</td></tr>';
		...
		if($field=='items_sort_orderby'){
			print'<label>';
				print "<select name='".WPPIZZA_SLUG."[".$options_key."][".$field."]' />";					
					print"<option value='menu_order' ".selected($wppizza_options[$options_key][$field],"menu_order",false).">".__('Default (as set by "Order" attribute)', 'wppizza-admin')."</option>";
