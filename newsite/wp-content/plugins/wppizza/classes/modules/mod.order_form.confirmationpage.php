		...
		********************************/		
		...
		if($help){
			$settings['help'][$this->section_form_enabled][] = array(
				...
				'description'=>array(
					__('If you require a final confirmation page, enable this option and save. Additional settings will be made available for this final confirmation page', 'wppizza-admin')
				)
			);
		}
		...
		if(!empty($wppizza_options[$this->section_key]['confirmation_form_enabled'])){
			...
			if($help){		
				$settings['help'][$this->section_form_localize][] = array(
					...
					'description'=>array(
						__('Several additional text, fields, buttons etc are being used on the final confirmation form.', 'wppizza-admin'),
