...
class WPPIZZA_MARKUP_MAINCART{
	...
	*/
	function summary_markup($order_formatted, $type){
		...
		*/
		$markup .= WPPIZZA() -> markup_maincart -> get_cart_json($order_formatted, $type);
...
******************************************************/
	...
	*/
	function get_cart_json($order_formatted = false, $type = false){
		...
		/*
			dont do anything if shop is closed
			or its not the orderpage or has no cart on it
		*/
		if( !wppizza_is_shop_open() || !in_array($type, array('minicart', 'cart', 'orderpage')) ){
			return '';
		}

		/*
			create json vars, distinctly enable on orderpage
		*/
		if($json_cart_contents === null){
			...
			******/
			...
			/******
				nothing in cart , bail right here
			******/
			...
			*******/
			$html_element = "<input id='".WPPIZZA_SLUG."-cart-json' type='hidden' value='".json_encode($json_cart_contents, JSON_FORCE_OBJECT)."' />";
