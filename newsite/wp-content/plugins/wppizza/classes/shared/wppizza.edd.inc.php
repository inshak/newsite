...
********************************************************************************/
	...
	function edd_for_plugins_markup($option_name = false, $echo = true){
		...
		**/
		...
		$markup.='<br/>'.__('Please note: entering and activating the licence is optional, but if you choose not to do so, you will not be informed of any future bugfixes and/or updates.', 'wppizza-admin').'<br />';
...
********************************************************************************/
	...
	function wppizza_gateways_edd($wppizza_registered_gateways){
		...
		foreach($wppizza_registered_gateways as $gateway_ident=>$wppizza_gateway_object){
			...
			if(method_exists($wppizza_gateway_object, 'gateway_edd_sl') && has_filter('wppizza_gateway_filter_settings_'.$gateway_ident.'')){
				...
				**/
				$gateway_edd_status = $gatewayEddSl->gateway_edd_toggle($wppizza_gateway_object->gatewayOptions, $wppizza_gateway_object->gatewayOptionName, $gateway_edd_sl['name'], $gateway_edd_sl['url']);
				if(!empty($gateway_edd_status['license'])){
					$this->gateways_edd_status[$gateway_edd_status['license']] = $gateway_edd_status;
				}

				/**add checked licence status filter to update options on save of gateways options**/
				add_filter('wppizza_filter_gateway_options_validate_'.strtolower($gateway_ident).'', array( $this, 'gateway_edd_options_validate'));
			}
		}
	}
	/***********************************************
	...
	***********************************************/
	function gateway_edd_options_validate($gateway_options){
		...
		$gateway_options['GatewayEDDLicense']['status']		=	empty($this->gateways_edd_status[$licence_key]['status']) ? '' : $this->gateways_edd_status[$licence_key]['status'];
		$gateway_options['GatewayEDDLicense']['error']		=	empty($this->gateways_edd_status[$licence_key]['error']) ? '' : $this->gateways_edd_status[$licence_key]['error'];
	...
	***********************************************/
	function gateway_edd_settings($gatewaySettings, $gatewayOptions, $gatewayOptionsName, $type){
		...
		if($type=='edit'){
			...
			$gwSettings['_gateway_edd']['descr'] = isset($gatewayOptions['GatewayEDDLicense']) ? $this->gateway_edd_licence_status($gatewayOptions['GatewayEDDLicense'], $gatewayOptionsName) : '';
	...
	***********************************************/
	function gateway_edd_licence_status($licence_status, $gateway_options_name){
		...
		***/
		...
		$output.="</p>";
		$output.="<span class='description' style='display:block;clear:both;padding-top:5px'>".__('Please note: entering and activating the licence is optional, but if you choose not to do so, you will not be informed of any future bugfixes and/or updates.', 'wppizza-admin')."</span>";
