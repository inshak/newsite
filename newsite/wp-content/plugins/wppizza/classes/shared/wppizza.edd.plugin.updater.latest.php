...
class WPPIZZA_EDD_SL_UPDATER {
	...
	public function __construct( $_api_url, $_plugin_file, $_api_data = null ) {
		...
		/* 
			use filename if it's a plugin that has no directory
			i.e /plugins/some-plugin-slug.php
		*/
		if(basename(WP_PLUGIN_DIR) === basename(dirname($_plugin_file)) ){
			$set_plugin_slug = basename( $_plugin_file, '.php' );
		}
		/* 
			use directory name if it's a plugin that is in 
			its own directory with some .php that does not correspond
			to the actual plugin slug 
			i.e /plugins/some-plugin-slug/index.php
		*/		
		else{
			$set_plugin_slug = basename( dirname($_plugin_file), '.php' );
		}		

		$this->api_url     = trailingslashit( $_api_url );
		$this->api_data    = $_api_data;
		$this->name        = plugin_basename( $_plugin_file );
		$this->slug        = $set_plugin_slug;
