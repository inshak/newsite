			...
			************************************************************************/
			...
			if(count($getWppizzaMenuItems)>0){
				...
				foreach($getWppizzaMenuItems as $menuItem){
					...
					$sizes = isset($meta['sizes']) && is_array($meta['sizes'])  && isset($wppizza_options['sizes'][$meta['sizes']]) ? $wppizza_options['sizes'][$meta['sizes']] : array();
					/*loop through sizes*/
					if(is_array($sizes)){
					foreach($sizes as $sizekey=>$size){
						/*make key from id and size*/
						$miKey = $menuItem->ID.'.'.$sizekey;
						$unsoldMenuItems[$miKey] = array('ID' => $menuItem->ID, 'title' => $menuItem->post_title, 'sizekey' => $sizekey, 'price_label' => $size['lbl'] );
					}}
				}
			}
		...
		*/
		if($dashboard_widget){
			...
			*/
			...
			#$data['totalsByPaymentType'] = $orderTotalsRes ;
			#$data['totalsByRegisteredUser'] = $orderTotalsUsersRes ;
		}
