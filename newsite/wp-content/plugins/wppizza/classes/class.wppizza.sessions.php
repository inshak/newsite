...
*
*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*/
	function sort_and_calculate_cart($is_checkout = null, $recalculate_all = false, $trace = false){
		...
		**/
		...
		if( $calc === 0 || !empty($recalculate_all) ){
			...
			* items - discount + delivery (0 when pickup) + taxes(to add) + handling
			***************************************/
			$total_before_tips = $total_price_items - ( $discount + $additional_discounts_sum) + $delivery_charges + $taxes_to_add + $handling_charges;
...
****************************************************************************************************************/
	...
	******************************************************/
	function maybe_init_sessions() {
		...
		/**********************************************************************
			skip session starts (in fact close it immediately) in health check
			(as WP5.2+ will complain in health check due to
			blocking php sessions)
			at some point we'll move away from php sessions altogether
		***********************************************************************/
		$is_wp_loopback_healthcheck = false;
		//admin
		if(is_admin()){
			if((!empty($_SERVER['REQUEST_URI']) && basename($_SERVER['REQUEST_URI']) == 'site-health.php') || (!empty($_POST['action']) && strtolower(substr($_POST['action'], 0 , 12)) == 'health-check')){
	    		$is_wp_loopback_healthcheck = true;
			}
		}
		//!admin - look for 'wp-site-health' in REQUEST_URI
		if(!is_admin() && !empty($_SERVER['REQUEST_URI'])){
			$strpos = strpos($_SERVER['REQUEST_URI'], 'wp-site-health');
			if($strpos !== false){
				$is_wp_loopback_healthcheck = true;
			}
		}
		/*
			for healthcheck scripts, just start and close session and return
			immediately without going any further
		*/
		if($is_wp_loopback_healthcheck){
	    	if (!session_id()) {
		...
		if(!empty($_POST['action']) && $_POST['action'] == 'wppizza_json' && !empty($_POST['vars']['type']) && $_POST['vars']['type'] == 'changegateway'){
			...
			*/
			if(!empty($_POST['vars']['gateway_selected']) ){
				//uppercase key
				$gw = wppizza_validate_alpha_only($_POST['vars']['gateway_selected']);
				//lowercase value !!
