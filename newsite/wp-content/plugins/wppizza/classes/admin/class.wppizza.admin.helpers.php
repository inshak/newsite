...
class WPPIZZA_ADMIN_HELPERS{
	...
	***********************************************************/
	function admin_pagination($results, $max_per_page, $getParam=false){
		...
		/** pages only (with links) **/		
		$pagination['pages']=!empty($pagination_pages) ? $pagination_pages : '';
		/**html pagination top**/
		$pagination['html_top'] = '<div class="widefat '.WPPIZZA_SLUG.'-pagination '.WPPIZZA_SLUG.'-pagination-top">';
		$pagination['html_top'] .= '<span class="'.WPPIZZA_SLUG.'-pagination-left">'.$pagination['on_page'].' '.__('of','wppizza-admin').' '.$pagination['total_count'].'</span>';
		$pagination['html_top'] .= '<span class="'.WPPIZZA_SLUG.'-pagination-right">'.$pagination['pages'] .'</span>';
		$pagination['html_top'] .= '</div>';
		/**html pagination top**/
		$pagination['html_bottom'] = '<div class="widefat '.WPPIZZA_SLUG.'-pagination '.WPPIZZA_SLUG.'-pagination-bottom">';
		$pagination['html_bottom'] .= '<span class="'.WPPIZZA_SLUG.'-pagination-left">'.$pagination['on_page'].' '.__('of','wppizza-admin').' '.$pagination['total_count'].'</span>';
		$pagination['html_bottom'] .= '<span class="'.WPPIZZA_SLUG.'-pagination-right">'.$pagination['pages'] .'</span>';
		$pagination['html_bottom'] .= '</div>';
	...
	function admin_pagination_links($current, $total, $pagedParam, $getParam=false){
		...
		**/
		...
		if(is_array($getParam)){
			...
			$getParam = wppizza_validate_array($getParam, 'wppizza_validate_string');/*sanitize array keys/values*/
	...
	***********************************************************/
	function admin_orderform_enabled_formfields($location){
		...
		/*
			...
			for gateway or plugins mapping
		*/
		if($location == 'gateways' || $location == 'plugins'){
					...
					*/
					if(!empty($template_options[$template_id])){
						...
						*/
						...
						$template_parameters = WPPIZZA() -> templates_email_print -> get_template_email_plaintext_sections_markup($order, $template_values, $template_type, '', $template_id );						
