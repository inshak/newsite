...
function wppizza_gmap_map_latlng($lat, $lng, $apiKey){
	...
	*/
	$args = array(
		'apiUrl' => 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.(float)trim($lat).','.(float)trim($lng).'&sensor=false&key='.$apiKey,
		'authorization' => false,
		'custom_headers' => array(),
		'content_type' => false,
		'parameters' => array(),
...
function wppizza_remote_send($parameters = array('apiUrl' => false , 'authorization' => false, 'custom_headers' => array(), 'content_type' => false, 'parameters' => array(), 'method' => 'POST') ){
	...
	*/
	...
	$json_encode_body = $content_type ==  'application/json; charset=utf-8' ? true : false;
